...
public class Repository
{
	...
	public Task<IEnumerable<Episode>> Search(string searchTerm) => Task.FromResult(
		Episodes.Where(e =>
			e.Title.Contains(searchTerm, StringComparison.OrdinalIgnoreCase) ||
			e.Number.ToString().Contains(searchTerm) ||
			e.PublishDate.ToString().Contains(searchTerm)
		));
}
